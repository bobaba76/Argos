...
- **Stale-pending review sweep** (#10): the four `stale_review_*` config keys are now consumed by a periodic daemon thread (`_stale_sweep_loop` in `provider_session.py`) started during `initialize()`. Each tick re-reviews `pending` candidates older than `stale_review_min_age_min`, capped at `stale_review_max_batch`, on `stale_review_interval_min` cadence. Reuses the same `_review_candidate` path (identical decision map + no-auto-promotion invariant). 8 tests (`test_stale_review_sweep.py`). Previously the keys were parsed but never read — dead config shipped with live docs.
