...
| `store_derived_identifier_mode` | `redact` | How plugin LLM calls built from STORED memories (graph typing, distillation, rollup) treat personal identifiers (emails, phone numbers, ID/card digit runs). `redact` (default): identifiers are masked before the payload leaves the machine - the call still runs on cleansed text. `gate`: refuse those calls when an identifier is present. `off`: send stored content unchanged (pre-#404 behavior). Conversation-derived calls (extraction, review, expansion) always refuse identifiers outright, regardless of this setting. **When to change:** set to `gate` for the strictest posture; only use `off` for diagnostics. |
...
| `approval_mode` | `auto` | Write policy for background saves (extraction, watcher, external API). `auto`: saves materialize immediately - medium/high-risk signals are stamped as an `unreviewed` trust class (rank-penalized by at most 3 positions at retrieval, never auto-promoted, resolvable only by an explicit human action). `human`: v1 behavior - nothing becomes active without human approval. Invalid values fail closed to `human`. Per-tenant override in the tenant's cell config. **When to change:** set to `human` for deployments that require a human gate on every write. |
...
| `graph_drift_check_interval_min` | `360` | How often the in-service DuckDB-to-graph drift check runs (minutes). `0` disables the watch. Uses the same probe as `scripts/reconcile_graph.py` (#329). **When to change:** lower for faster drift detection; `0` to disable the watch. |
| `graph_drift_auto_heal` | `true` | When drift is detected, re-index missing memories into the graph automatically (idempotent; the graph is derived data and source records are never modified). **When to change:** set to `false` for detect-only mode. |
...
| `retention_enabled` | `false` | POPIA retention enforcement. At session end, records whose record-class retention period (per `retention_policies`) has passed get `expires_at` stamped to the retention deadline, so they drop out of retrieval via the existing TTL machinery. Deterministic, zero-LLM, no new scheduler. Deletion itself happens via the erase-request workflow with a deletion receipt. **When to change:** enable for deployments with per-category data-retention obligations. |
| `retention_policies` | `{}` | JSON object mapping record category to retention period in days, e.g. `{"context_note": 730, "event": 1825}`. Categories without a policy are never expired by retention. Only consulted when `retention_enabled` is `true`. **When to change:** set per-category retention periods matching your data-retention policy. |
