...
| MEMORY_SYSTEM.md:102 + CONFIG_REFERENCE.md:79-82 + UI label "Stale-pending sweep": "periodically re-reviews proposals pending too long" | **Implemented (#10).** The four keys (`stale_review_sweep_enabled`, `stale_review_interval_min`, `stale_review_min_age_min`, `stale_review_max_batch`) are consumed by a periodic daemon thread (`_stale_sweep_loop` in `provider_session.py`) started during `initialize()`. Each tick lists `pending` candidates older than `min_age_min`, caps at `max_batch`, and re-reviews via the same `_review_candidate` path (identical decision map + no-auto-promotion invariant). Previously configured-but-unimplemented; the sweep was wired up and tests added in `test_stale_review_sweep.py`. |
