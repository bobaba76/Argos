...
|-----|-----------|--------------------|
| A (dev fork) | `C:\Users\michael\Documents\Github\Hermes\hybrid_memory_plugin` | Has `.git` at Hermes root (recheck why `git -C` silently failed). Plugin files are mostly **Aug 6–9 vintage** — the handoff's "A is where commits happen" no longer matches: recent commits (Aug 22–23) were made in the Argos repo itself. |
