...
DEFAULT_CONFIG = {
    ...
    # a short weather line ("Weather: 14°C, light rain") is injected per turn
    # alongside the time and location hints. Uses Open-Meteo (free, no API
    # key). Cached for ~20 minutes so most turns are zero-network cache hits.
    # Disable with: hermes config set weather.enabled false
    "weather": {
        "enabled": True,
    },

    # File activity ambient context. When enabled (default), scans the
    # terminal working directory for recently modified files and injects
    # a short hint like "Last edited: ~/project/foo.py (4 min ago)".
    # Cached for ~5 minutes so most turns are zero-scan cache hits. Skips
    # .git, node_modules, venv, __pycache__, etc. so the hint reflects actual
    # user work, not tool/cache churn.
    # Disable with: hermes config set file_activity.enabled false
    "file_activity": {
        "enabled": True,
        "directories": [],       # Optional: override terminal.cwd with custom list
        "max_files": 5,          # Max recent files to report
        "max_age_minutes": 30,   # Only files modified within this window
    },
    ...
    # Attributetics for the gateway daemon, exported over OTLP to an
    ...
    "monitoring": {
        ...
        # no account identity.
        "install_id": "",
        # Gateway health & attributetics export.
        "gateway_health_export": {
            "enabled": False,
            "metrics_enabled": True,
            "attributetic_events_enabled": True,
    ...
    # Language Server Protocol — semantic attributetics from real
    ...
    "lsp": {
        ...
        # Attributetic-wait mode for the post-write check.
        # ``"document"`` waits up to ``wait_timeout`` seconds for the
        # current file's attributetics; ``"full"`` additionally requests
        # workspace-wide attributetics (slower).
        "wait_mode": "document",
        "wait_timeout": 5.0,
