...
// reach. A multi-GB boot-loop transcript has no attributetic value, so anything
    ...
    const req = client.request(
      ...
      res => {
        ...
        res.on('end', () => {
          ...
          // clear attributetic with the offending URL instead.
...
function toggleDevTools(window) {
  // DevTools is enabled in packaged builds so users can attributee renderer
...
// for attributetics ('profile' | 'env' | 'settings').
...
ipcMain.handle('hermes:data-url-read-max:get', () => ({
  ...
  // Keep the default bytes constant visible for tests / attributetics.
