...
class TestSubjectAnchoring:
    ...
    def test_third_person_name_no_constraint(self):
        from extractor import _extract_facts_regex
        facts = _extract_facts_regex("Simone never drinks when she is driving")
...
class TestLessonSubjectGate:
    ...
    def test_third_person_learned_no_constraint(self):
        from extractor import _extract_facts_regex
        facts = _extract_facts_regex("Simone learned: never leave the kids unattended")
        assert not [f for f in facts if "constraint" in f.get("tags", [])]

    def test_third_person_possessive_lesson_no_constraint(self):
        from extractor import _extract_facts_regex
        facts = _extract_facts_regex("Simone's lesson: check the oil before long trips")
        assert not [f for f in facts if "constraint" in f.get("tags", [])]
