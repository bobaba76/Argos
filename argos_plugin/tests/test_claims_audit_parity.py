"""Parity canary: the CLAIMS-AUDIT.md §2 "Test suite" row must track the suite.
...
disk). This test parses the quoted counts and compares them against
``scripts/count_test_fns.py`` so the row cannot silently rot again. A
tolerance is allowed so ordinary test additions don't fail the suite;
refresh the row (and its changelog entry) when the drift exceeds it.
"""
