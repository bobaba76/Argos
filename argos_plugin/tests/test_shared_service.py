...
def test_shared_store_review_candidate_forwards_keyword_arguments(tmp_path):
    ...
    try:
        ...
        reviewed = store.review_candidate(
            ...
            decision="approved",
            reason="confirmed",
            # #423: user-confirmed class must ride the gated channel;
            # the client auto-gates tool/manual claims (a no-class
            # call is server-derived to auto_review and cannot approve).
            review_source="tool",
        )
